...
- **Need:** Store `repoOwner` and `repoName` on each Run so future dashboard features can filter and group by repo.
...
- Add two nullable string columns (`repoOwner`, `repoName`) to the `Run` model in `prisma/schema.prisma`
...
| Actor | Interaction |
|-------|-------------|
| **Pipeline Client** | May include `repoOwner` and `repoName` in the telemetry POST body. Omission is valid. |
...
1. Pipeline Client sends a telemetry POST including `"repoOwner": "NewmanJustice"` and `"repoName": "murmur8_portal"`.
...
1. Pipeline Client sends a telemetry POST without `repoOwner`/`repoName`.
...
- If `repoOwner` is present but not a string, return a validation error.
...
- **State-creating:** The Run record may now be created with `repoOwner` and `repoName` populated.
...
| Rule | Description |
|------|-------------|
| **R-REPO-1** | `repoOwner` and `repoName` are independently optional. Either, both, or neither may be provided. |
...
- **Performance:** No index is needed on these columns in v1. If repo-based filtering is added later, a composite index `(userId, repoOwner, repoName)` may be warranted.
...
- `repoOwner` corresponds to the GitHub user or organisation login (e.g. `"NewmanJustice"`).
...
- The Run domain concept (System Spec section 5) lists key fields; `repoOwner` and `repoName` are a natural extension of the provenance metadata already present (`commitHash`).
- No invariants or governing rules are affected.
- **Proposed system spec update (deferred):** Add `repoOwner` (String?) and `repoName` (String?) to the Run field table in section 5. This should be applied after this feature lands.
