...
export function computeInsights(runs) {
  ...
  if (totalRuns === 0) {
    return {
      ...
      last7Days: 0, last30Days: 0, topSlugByRunCount: null, topSlugByCost: null, avgFeedbackRating: null,
    };
  }
  ...
  const slugRunCounts = new Map();
  const slugCostTotals = new Map();
  for (const run of runs) {
